                        ...
                        <div class="card-body">
                            ...
                            @if($cashOnDeliveryBtnShow && $cash_on_delivery['status'] || $digital_payment['status']==1)
                                <div class="d-flex flex-wrap gap-3 mb-3">
                                    ...
                                    <div id="cod-for-cart" class="w-100">
                                        <div class="card cursor-pointer w-100">
                                            <form action="{{route('checkout-complete')}}" method="get" class="needs-validation w-100" id="cash_on_delivery_form">
                                                <label class="m-0 w-100">
                                                    <span class="btn btn-block click-if-alone d-flex gap-2 align-items-center cursor-pointer w-100">
                                                        <img width="40" src="{{ theme_asset(path: 'public/assets/front-end/img/icons/money.png') }}" alt="">
                                                        <div class="d-flex flex-column align-items-start">
                                                            <span class="fs-20 font-weight-bold" style="color: black;">{{ translate('cash_on_Delivery') }}</span>
                                                            <span class="fs-12">Paid with any method at delivery</span>
                                                        </div>

                                                        <input type="radio" id="cash_on_delivery" class="custom-radio ms-auto">
                                                    </span>
                            ...
                            @if ($digital_payment['status']==1)
                                ...
                                @foreach ($payment_gateways_list as $payment_gateway)
                                    <div id="cod-for-cart" class="w-100">
                                        <div class="card cursor-pointer w-100">
                                            <form method="post" class="digital_payment w-100" id="{{($payment_gateway->key_name)}}_form" action="{{ route('customer.web-payment-request') }}">
                                                @csrf
                                                ...
                                                <label class="btn btn-block click-if-alone d-flex gap-2 align-items-center cursor-pointer w-100" style="margin-bottom: 0px;">
                                                    <!-- Image and content -->
                                                    <img width="50" src="{{dynamicStorage(path: 'storage/app/public/payment_modules/gateway_image')}}/{{ $payment_gateway->additional_data && (json_decode($payment_gateway->additional_data)->gateway_image) != null ? (json_decode($payment_gateway->additional_data)->gateway_image) : ''}}" alt="">
                                                    <div class="d-flex flex-column align-items-start">
                                                        <span class="text-capitalize form-check-label font-weight-bold" style="color: black;">
                                                        ...
                                                        <span class="fs-12">Scan to pay with any banking app</span>
                                                    </div>

                                                    <!-- Radio button aligned at the end -->
                                                    <input type="radio" id="{{($payment_gateway->key_name)}}" name="online_payment" class="form-check-input custom-radio ms-auto" value="{{($payment_gateway->key_name)}}">
                                                </label>
                                            </form>
                                        </div>
                                    </div>
