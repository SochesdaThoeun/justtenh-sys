...
class ProductController extends Controller
{
    ...
    public function getProductList(Request $request): JsonResponse
    {
        ...
        $products = Product::with(['digitalVariation'])->withCount('reviews')->where(['added_by' => 'seller', 'user_id' => $seller['id']])
            ...
            ->when($request->has('category_id') && $request['category_id'] != 0, function ($query) use ($request) {
                ...
                $query->where(function ($query) use ($category_id) {
                    // Query for category_id (direct match)
                    $query->where('category_id', $category_id)
                        // Or query for category_ids array (contains match)
                        ->orWhere(function ($q) use ($category_id) {
                            $q->whereJsonContains('category_ids', ['id' => $category_id])
                              ->orWhereJsonContains('category_ids', ['id' => (string)$category_id]);
                        });
                });
            })
    ...
    public function add_new(Request $request)
    {
        ...
        $requestChoice = json_decode($request['choice'], true);
        ...
        if ($request->has('choice_no')) {
            foreach ($requestChoiceNo as $key => $no) {
                ...
                $options[] = json_decode($request[$name], true);
            }
        }
    ...
    public function update(Request $request, $id): JsonResponse
    {
        ...
        if ($request->has('choice')) {
            foreach ($requestChoiceNo as $key => $no) {
                $str = 'choice_options_' . $no;
                $item['name'] = 'choice_' . $no;
                $item['title'] = $requestChoice[$key ]; //Update
                $item['options'] = $request[$str];
                $choice_options[] = $item;
            }
        }
        ...
        if ($request->has('choice_no')) {
            foreach ($requestChoiceNo as $key => $no) {
                $name = 'choice_options_' . $no;
                $options[] = json_decode($request[$name], true); //Update
            }
        }
        //Generates the combinations of customer choice options
        $combinations = Helpers::combinations($options);
