        ...
        if (!App::runningInConsole()) {
            ...
            try {
                if (Schema::hasTable('business_settings')) {
                    ...
                    // Ensure web_config is always an array to prevent foreach errors
                    // Clean up web_config if it has been corrupted by debugging output
                    if (is_string($web_config) && strpos($web_config, 'sf-dump') !== false) {
                        // Web config was dumped, recreate it
                        $web_config = [
                    ...
                    } elseif (!is_array($web_config)) {
                        // If web_config is corrupted in another way, regenerate basic structure
                        $web_config = [
                            'primary_color' => $data['primary'] ?? '#132442',
                            'secondary_color' => $data['secondary'] ?? '#000000',
                            'primary_color_light' => $data['primary_light'] ?? '#CFDFFB',
                        ...
                        // Ensure array elements are proper arrays
                        $web_config['social_media'] = is_array($web_config['social_media'] ?? null) ? $web_config['social_media'] : [];
                        $web_config['currencies'] = is_array($web_config['currencies'] ?? null) ? $web_config['currencies'] : [];
                        $web_config['shops'] = is_array($web_config['shops'] ?? null) ? $web_config['shops'] : [];
                        $web_config['socials_login'] = is_array($web_config['socials_login'] ?? null) ? $web_config['socials_login'] : [];
                        $web_config['featured_deals'] = is_array($web_config['featured_deals'] ?? null) || ($web_config['featured_deals'] ?? null) instanceof \Illuminate\Support\Collection ? $web_config['featured_deals'] : collect([]);
                    }
                    
                    // Add a view composer to ensure web_config is always clean
                    View::composer('*', function ($view) use ($web_config) {
                        // Check if web_config is corrupted in the view data
                        $viewData = $view->getData();
                        if (isset($viewData['web_config']) && is_string($viewData['web_config']) && strpos($viewData['web_config'], 'sf-dump') !== false) {
                            // Override with clean web_config
